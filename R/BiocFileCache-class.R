...
#' \itemize{
#'    \item{'cache': }{character(1),  Describes the On-disk location (directory
#' path) of the cache}
...
#'   \item{'rname': }{resource name. This is given by the user when a resource is
#' added to the cache. It does not have to be unique and can be updated at
#' anytime. We recommend descriptive key words and identifers.}
#'   \item{'create_time': }{The date and time a resource is added to the cache.}
#'   \item{'access_time': }{The date and time a resource is utilized within the
#' cache. The access time is updated when the resource is updated or accessed}
#'   \item{'rpath': }{resource path. This is the path to the local (on-disk) file}
#'   \item{'rtype': }{resource type. Either "local" or "web", indicating if the
#' resource has a remote origin}
#'   \item{'fpath': }{If rtype is "web", this is the link to the remote
#' resource. It will be utilized to download or update the remote data}
#'   \item{'last_modified_time': }{For a remote resource, the last_modified (if
#' available) information for the local copy of the data. This information is
#' checked against the remote resource to determine if the local copy is stale
